...
function fetchArchives() {
    ...
    try {
        years.forEach(function(y) {
            ...
            saveArchives(`archives_subjects/${y}.csv`, processed)
        })
...
function getInstitutionData(row, year, idx) {
    try {
        ...
        const html = fs.readFileSync(`institutions_subjects/${year}_${idx}.html`, { encoding: "UTF8" })
...
function buildRow(tableRow, institution) {
    ...
    return {
        ...
        alumni_score: getScore(cells[4]),
        award_score: getScore(cells[5]),
        hici_score: getScore(cells[6]),
        pub_score: getScore(cells[7]),
        top_score: getScore(cells[8])
    }
}
...
function getCountry(cell) {
    ...
    return normalizeCountry(imgSrc.substring(imgSrc.lastIndexOf('/') + 1).split(".")[0])
}

function normalizeCountry(country) {
    return country.replace(/([A-Z][a-z]+)([A-Z].)/,"$1 $2")
}
...
function fetchArchive(year) {
    ...
    return fetch(`http://archive.shanghairanking.com/FieldLIFE${year}.html`).text()
}

function paramsError() {
    console.error("node ./fetch_archives_for_subjects.mjs")
}
